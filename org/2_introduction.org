...
- These are *not* data structures:
  ...
  7. Pointers (=int *p = &i;=) - variables that hold addresses
...
* Containers vs. Container adaptors

- Since ~vector~ has push and pop functions, isn't it just a stack, too?
  #+begin_quote
  No - a ~vector~ is a container, and a ~stack~ is a container 
  adaptor. Both of these are available in the C++ Standard Template
  Library.
  #+end_quote  
- What's a container?
  #+begin_quote
  A *container* owns its own data, defines how elements are organized
  (e.g. array-like for a ~vector~, or linked like a ~list~).
  #+end_quote  
- What's a container adaptor?
  #+begin_quote
  A *container adaptor* is only a wrapper around a container - built on
  top of a container (~stack~ and ~queue~ default to ~deque~, and
  ~priority_queue~ defaults to ~vector~), and does not expose the full
  container interface - e.g. the ~stack~ only allows push, pop, top, the
  ~queue~ only push, pop, front, back.

  A container adaptor also enforces an ADT discipline: LIFO for ~stack~,
  FIFO for ~queue~, priority ordering for ~priority_queue~.
  #+end_quote
